...
export const Departments = () => {
  ...
  // turns
  const resetTakenTurn = useCallback(async () => {
    try {
       await request(
        `/api/services/department/resetTurns/${department?._id}`,
        "PUT",
        {},
        {
          Authorization: `Bearer ${auth.token}`,
      ...
      notify({
        title: `${department.name} ${t("bo'limi yangilandi")}!`,
        description: "",
        status: "success",
      });
    } catch (error) {
  ...
  const clearTakenTurs = () => resetTakenTurn();
  return (
    <>
      {loading ? <Loader /> : ""}
        ...
        <div className="row gutters">
          ...
          >
            <div className="md:flex flex-wrap">
              ...
              <div className="flex-none md:w-1/2 mt-1">
                <div className="text-[16px] flex flex-col w-80">
                  <label>{t("Olingan navbatlarni tozalash")}</label>
                  <Button
                    onClick={clearTakenTurs}
                    type="button"
                    className="form-control w-80 gap-x-2"
                  >
                    <FaRegTrashAlt /> Tozalash
                  </Button>
                </div>
              </div>
              <div className="flex-none md:w-1/2  mt-1">
                <div className="text-[16px]">
                  <label>{t("Navbat soni")}</label>
                  <input
                    style={{ minWidth: "25%" }}
                    value={department?.dayMaxTurns || ""}
                    onKeyUp={keyPressed}
                    onChange={(e) =>
                      setDepartment({
                        ...department,
                        dayMaxTurns: Number(e.target.value || 0),
                      })
                    }
                    type="text"
                    className="form-control w-80"
