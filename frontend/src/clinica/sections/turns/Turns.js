...
const Turns = () => {
  ...
  const getDepartmentsTurn = useCallback(async () => {
    try {
      ...
      <div className="bg-white pt-[10px]">
        ...
        <div className="min-h-full px-6 pt-[10px] pb-[100px] grid grid-cols-3 lg:grid-cols-4 2xl:grid-cols-6 gap-8 bg-white">
          {departmentsTurn.map((department, ind) => {
            return (
              <div
                className="col-span bg-white rounded-md flex flex-col items-center gap-2 py-4 border-[3px] border-alotrade"
                key={ind}
              >
                <div className="w-full font-bold flex justify-center items-end">
                  <p className="text-green-500 font-semibold text-[21px]">
                    {t("NAVBAT")}:
                  </p>
