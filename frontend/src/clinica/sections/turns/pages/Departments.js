...
const Departments = () => {
  ...
  const handleChangeActiveCheckbox = (e, index) => {
    ...
    saveLocalStorageSelectedDepartmentsNames();
  };
  ...
  const saveLocalStorageSelectedDepartmentsNames = () => {
    localStorage.setItem(
      "selected_departments_names",
      JSON.stringify(
        departmentsTurn
          .filter((item) => item.active)
          .map((item) => {
  ...
  const saveLocalStorageSelectedOnlineClients = (updatedDepartmentsTurn) => {
    ...
    const selectedClient = updatedDepartmentsTurn.find((item) => {
      return item.onlineActive;
    });
    ...
    localStorage.setItem(
      "online_department_name",
      JSON.stringify(selectedClient ? selectedClient.name : "")
    );
  };
                    ...
                    <tr key={item?._id} className={"text-black"}>
                      ...
                      <td className={"text-base border text-center"}>
                        <Checkbox
                          ...
                          onChange={(e) =>
                            handleOnlineClientsCheckbox(e, index)
                          }
                        />
                      </td>
                    </tr>
