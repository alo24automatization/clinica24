...
import {
  ...
  checkProductsData,
  checkServicesData,
} from "../../reseption/offlineclients/checkData/checkData";
...
export const DoctorClients = () => {
  ...
  const getUserAccessDetails = async () => {
    try {
      const response = await request(
        `/api/user/getUserById/${auth?.user?._id}`,
        "GET"
      );
  ...
  const changeAccept = (e) => {
    ...
    if (e.target.value === "not") {
      searching = [...searchStorage].filter(
        (connector) =>
      ...
      setDoctorClients(
        [...searchStorage]
          ...
          .sort(
            (a, b) =>
              a.services.filter(
                (s) => s.department._id === auth?.user?.specialty?._id
              )[0].turn -
              b.services.filter(
                (s) => s.department._id === auth?.user?.specialty?._id
              )[0].turn
      ...
      <div className="bg-slate-100 content-wrapper px-lg-5 px-3">
        <div className="row gutters">
          <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div className="flex">
              {doctorAccessDetails &&
              doctorAccessDetails?.accessCreateClient ? (
