...
export const TableClients = ({
  ...
  return (
    <div className="border-0 shadow-lg table-container">
        ...
        <div className="table-responsive">
          <div className="bg-white flex gap-6 items-center py-2 px-2">
            ...
            <div className="flex items-center gap-4">
              <input
                onKeyDown={(e) =>
                  e.key === "Enter" && getClientsByBorn(e.target.value)
                }
                type="date"
                name="born"
            ...
            >
              {startedFilter ? <FontAwesomeIcon icon={faClose} /> : t("Filtr")}
            </button>
            ...
            >
              ...
              <ModalContent>
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="grid grid-cols-2 gap-x-4">
                      ...
                      <Select
                        isMulti
                        onChange={(value) =>
                          handleChangeFilter(value, "complaints")
                        }
                      ...
                      <Select
                        isMulti
                        onChange={(value) =>
                          handleChangeFilter(value, "diagnostics")
                        }
                    ...
                    <div className="grid grid-cols-2 gap-x-3">
                      <FormControl>
                        <FormLabel>{t("Yosh tanlang")}</FormLabel>
                        <div className="flex items-center gap-x-2">
                          <input
                            onChange={(e) =>
                              handleChangeFilter(e.target.value, "from_age")
                            }
                            className="form-control"
                            placeholder="Yosh dan"
                          />
                          <input
                            onChange={(e) =>
                              handleChangeFilter(e.target.value, "to_age")
                            }
                            className="form-control"
                            placeholder="Yosh gacha"
                          />
                        </div>
                      </FormControl>
                      <FormControl
                        onChange={(e) =>
                          handleChangeFilter(e.target.value, "gender")
                        }
                      >
                    ...
                    <div className="grid grid-cols-2">
                      <FormControl
                        onChange={(e) =>
                          handleChangeFilter(e.target.value, "national")
                        }
                      >
                        <FormLabel>{t("Millati")}</FormLabel>
                        <RadioGroup>
                          <Stack direction="row">
                    ...
                    <div className="grid grid-cols-2">
                      ...
                      <FormControl
                        onChange={(e) =>
                          handleChangeFilter(e.target.value, "isDisability")
                        }
                      >
                        <FormLabel>{t("Nogironligi")}</FormLabel>
                        <RadioGroup>
                          <Stack direction="row">
                ...
                <ModalFooter className=" gap-x-3">
                  <Button onClick={closeFilterModal} colorScheme="red">
                    {t("Bekor qilish")}
                  </Button>
                  <Button
                    className=" !bg-alotrade py-1.5 text-white rounded-sm px-5 font-semibold text-base flex items-center justify-center"
                    onClick={() => handleStartFilter(clientFilterData)}
                  >
                    {t("Qiridish")}
                  </Button>
                </ModalFooter>
          ...
          <table className="table m-0" id="discount-table">
            <thead>
              <tr>
                ...
                <th className="border bg-alotrade text-[16px] py-1 w-[12%]"></th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  ...
                  >
                    <td
                      className={`${isDebt(
                        connector.payments
                      )} border text-[16px] py-1 font-weight-bold text-right`}
                      style={{ maxWidth: "30px !important" }}
                      onClick={() => {
                        const debt = connector.payments.reduce(
                    ...
                    {clientsType === "statsionar" && (
                      <td className="border text-[16px] py-1 text-right">
                        {connector?.connector?.room?.room?.type}{" "}
                        {connector?.connector?.room?.room?.number}{" "}
                        {connector?.connector?.room?.room?.place}
                      </td>
                    )}
                    <td className="border text-[16px] py-1 text-right">
                    ...
                    <td className="border text-[16px] py-1 text-center flex gap-[4px] items-center">
                      {loading ? (
                        ...
                        <button
                          ...
                          onClick={() =>
                            callClient(
                              connector?.client?._id,
                              connector?.services[0]?.department._id
                            )
                          }
      ...
      <Modal modal={modal} text={""} setModal={setModal} basic={debt} />
    </div>
  );
};
