...
module.exports.register = async (req, res) => {
  try {
    ...
    const checkStatsionarConnector =
      validateStatsionarConnector(connector).error;
    ...
    if (connector.probirka) {
      ...
      probirka =
        lastProbirka.length > 0
          ? Number(lastProbirka[0].probirka.substring(1)) + 1
          : 1;
    } else {
      probirka = 0;
    }
    ...
    if (offlineclient && offlineconnector) {
      const offline_connector =
        await OfflineConnector.findById(offlineconnector);
      ...
      for (const payment of offlinepayments) {
        ...
        if (payment.discount) {
          let offlinediscount = await OfflineDiscount.findById(
            payment.discount,
          );
...
const offlineToStatsionar = async (connector, client) => {
  ...
  const newclient = new StatsionarClient({
    ...client,
    id: "S" + client.id,
    fullname,
  });
...
module.exports.add = async (req, res) => {
  try {
    const { client, connector, services, products, counteragent, adver, room } =
      req.body;

    // Update StatsionarClient
    const updateClient = await StatsionarClient.findByIdAndUpdate(
      client._id,
      client,
    );

    // Find the OfflineClient by ID
    const updateClientCardNumber = await OfflineClient.findById(client._id);

    // Check if the document was found
    if (updateClientCardNumber) {
      // Update the card_number
      updateClientCardNumber.card_number = client.card_number;
      // Save the updated document
      await updateClientCardNumber.save();
    } else {
      // Handle the case where no document was found
      console.error(`OfflineClient with ID ${client._id} not found`);
      // throw new Error(`OfflineClient with ID ${client._id} not found`)
    }

    // Update StatsionarConnector
  ...
  try {
    ...
    if (type === "today") {
      connectors = await StatsionarConnector.find({
        ...
        .select(
          "client doctor createdAt services products dailys room diagnosis",
        )
        .populate("client")
        .populate({
          path: "services",
          select:
            "service serviceid accept refuse column createdAt tables turn connector client files department",
          populate: {
            path: "serviceid",
            select: "servicetype",
            populate: {
      ...
      connectors = await StatsionarConnector.find({
        ...
        .select(
          "client doctor createdAt dailys services products room diagnosis",
        )
        .populate("client")
        .populate({
          path: "services",
        ...
        .then((connectors) =>
          connectors.filter(
            (connector) => connector.room && connector.room.endday === null,
          ),
        );
    } else {
      connectors = await StatsionarConnector.find({
...
module.exports.getNurseProcient = async (req, res) => {
  try {
    ...
    const connectors = await StatsionarConnector.find({
      ...
      .lean()
      .then((connectors) =>
        connectors.filter((connector) => connector.room.room.nurseProcient),
      );
